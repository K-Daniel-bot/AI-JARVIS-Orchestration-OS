...
## AI JARVIS 오케스트레이션 에이전트 OS는 사용자의 입력 프롬프트 또는 음성을 깊이 분석하여 무엇을 요구를 하고 있는지 정확하게 오차율이 거의 없이 해석을 해서 사용자의 요구사항을 실행하도록 합니다.
...
## 에이전트 샌드박스 OS 제어 레이어 (직접 OS 접근 금지)
...
# 5) LangGraph 스타일 에이전트 라우팅 규칙 (핵심 조건표)
...
# 2️⃣ 복잡도 분류기 (에이전트팀 호출 여부 판단 엔진)

토큰 낭비 방지의 핵심입니다.

---

# 오케스트레이터가 만드는 “AI 작업 최적 환경”
...
### 1️⃣ 복잡도 분류기 (에이전트 팀 호출 여부 판단 엔진)
...
### 5️⃣ claude.md = 부트로더 규칙 명문화

150줄 전략을 시스템 계약으로 승격

---

## A. 오케스트레이터 에이전트 (중앙 조율)
...
# 공급망 (패키지 설치) 보안: “SBOM + 잠금 + 허용목록”

코드 구현 요청에서 가장 흔한 사고는 여기서 납니다.

## 3.1 패키지 설치는 게이트 + 정책 검사 필수
...
# 자격증명/비밀정보 보안: “절대 평문 금지”를 실행 레벨로

문서 원칙이 실제 실행에서 깨지지 않게 **기술적 강제**가 필요합니다.

## 4.1 비밀정보 마스킹 파이프라인 (필수)
...
## (A) 웹 사전검사 게이트 (URL 신뢰도/피싱 탐지)

* 웹 열기 전에 실행

## (B) 다운로드 승인 게이트 (다운로드 검사 후 승인)

* 스캔 결과와 위험 이유를 사용자에게 표시

## (C) 파괴적 작업 게이트 (삭제/대량 변경/권한 변경)

* 삭제, 포맷, 설정 변경 등은 별도 게이트

---

# 8) 정책 결정에 추가하면 좋은 필드 (스키마 확장)
...
# 3) 게이트 화면 실제 와이어 (3종 + 웹사전검사/다운로드/파괴적작업)

## 공통 게이트 카드 와이어 (기본 레이아웃)
...
# 디자인 토큰 보완 규칙 (JARVIS 느낌 유지 + 실사용성)

## 1) 색상 사용 규칙 강화 (제품 피로도 방지)
...
# 원격 조작 액션 애니메이션: “빠르게 처리하되, 눈으로 추적 가능”하게
...
## 2.5 원격 제어 애니메이션 상태 이벤트 (프론트 상태 모델 확장)
...
## 3.2 스캔/분석은 커서가 아니라 “HUD 진행상황”으로
...
# 보완 2: 예산-모델 연동 매트릭스 (토큰 예산 ↔ 모델 핫스왑)
...
# AB. 가역적 시스템 (모든 작업 되돌리기 가능 — 시스템 전체 가역성 보장)
...
```
  ...
  4차 추가 섹션:
  ═══════════════════════════════════════════════════════════
  AC. 심층 분석 보고서          시스템 전체 Gap Analysis (누락 기능 8건 + 보완 필요 10건)
  AD. OPENCLAW-style 실전 시나리오 (3종)
      AD-A. Slack Thread → 회의록 → Notion (L1)
      AD-B. Telegram → 주간보고 → Notion (L1)
      AD-C. API 상태 점검 → 문서화 → 알림 (L1 + L2)
```

---

# AC. JARVIS 오케스트레이션 에이전트 OS — 심층 분석 보고서

> 시스템 전체 설계를 깊이 분석하여 도출한 **누락 기능(Missing Features)**과
> **보완 필요 영역(Areas for Improvement)**을 체계적으로 정리한다.
> 이 분석은 기존 A~AB 섹션 전체를 대상으로 수행되었다.

---

## AC-1. 누락 기능 — 있으면 더 좋은 것

### AC-1-1. 플러그인/확장 생태계

현재 `plugins/` 디렉토리가 구조에 존재하지만, 플러그인 SDK, API 스펙, 샌드박싱 규칙이 부재하다.

```
  필요 항목                     설명
  ──────────────────────────────────────────────────────────────
  플러그인 매니페스트 스키마     이름, 버전, 필요 권한, Capability 요구사항
  플러그인 샌드박스              플러그인이 요청 가능한 Capability 범위 제한
  플러그인 마켓플레이스          커뮤니티 공유, 검증된 플러그인 목록
  플러그인 간 의존성 해결        버전 충돌 방지

  이유: Phase 5에서 Plugin Architecture를 언급하지만,
        실제 설계 없이 3rd party 확장을 허용하면 보안 구멍이 된다.
```

### AC-1-2. 맥락 기반 되돌리기/다시실행 이력

AB 섹션에 Reversible System이 존재하지만, 사용자 관점의 Undo 시각화가 부족하다.

```
  필요 항목                     설명
  ──────────────────────────────────────────────────────────────
  "의미 단위" 되돌리기          Run 단위가 아닌 "방금 리팩터링 전체를 되돌려줘"
  Branch-based Undo             Git처럼 되돌리기 분기 관리
  Undo Preview                  되돌리기 전 영향 범위 미리보기
```

### AC-1-3. 오프라인 우선 AI 기능

시나리오 9에서 "네트워크 끊김 상태"를 다루지만, 오프라인에서 AI 추론 자체가 작동하는 전략이 없다.

```
  전략                          설명
  ──────────────────────────────────────────────────────────────
  경량 로컬 모델 폴백            Claude API 불가 시 로컬 LLM (Ollama 등)으로 기본 작업
  오프라인 정책 평가             Policy Engine은 규칙 기반이므로 오프라인 가동 가능 — 명시 필요
  큐잉 + 동기화                 오프라인 작업을 큐에 저장, 온라인 복귀 시 Claude API로 재검증
```

### AC-1-4. 접근성

UI 설계가 상세하지만 접근성(a11y)에 대한 언급이 전혀 없다.

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  스크린 리더 호환               Gate 버튼, 타임라인 노드, Risk 배지의 ARIA 레이블
  키보드 네비게이션              모든 Gate 승인/거부가 키보드만으로 가능
  고대비 모드                   Glass HUD 테마가 시각 장애 사용자에게 읽기 어려울 수 있음
  음성 전용 모드                X-2 음성 피드백 확장 → 완전한 핸즈프리 조작
```

### AC-1-5. 크로스 플랫폼 추상화 레이어 상세화

Windows DPAPI / macOS Keychain은 언급되지만, OS별 Action API 구현 차이에 대한 추상화 전략이 부족하다.
...
```
  ...
  필요: OS별 어댑터 인터페이스 설계, Linux 지원 계획, 크로스 플랫폼 테스트 전략
```

### AC-1-6. 속도 제한 및 할당량 관리 (외부 API 관점)

Claude API 호출에 대한 Rate Limit 핸들링 전략이 K-5 에러 복구에 간단히 언급되지만 체계적이지 않다.

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  API 키별 RPM/TPM 추적          분당/초당 요청량 모니터링
  큐 기반 요청 스케줄링          burst 방지, 우선순위 큐
  다중 API 키 로테이션           팀 환경에서 키 분산
  비용 알림 임계값               실시간 비용 경고
```

### AC-1-7. 원격 측정 및 분석 (자체 시스템 모니터링)

보고서 시스템(T 섹션)이 존재하지만, 시스템 자체의 성능 텔레메트리가 없다.

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  에이전트별 평균 응답 시간       병목 탐지
  Gate 승인까지 평균 대기 시간   UX 최적화
  파이프라인 병목 지점 자동 탐지  성능 개선 포인트 식별
  메모리/디스크 사용량 추세       리소스 관리
```

### AC-1-8. 자연어 정책 정의

P 섹션에서 NL→Policy 변환을 다루지만, 더 직관적인 사용자 인터페이스가 필요하다.

```
  예시:
  자연어: "업무 시간(9-18시) 외에는 모든 웹 접근을 차단해줘"
  → 자동 변환:
  {
    "rule": "network.deny_all",
    "condition": "time.outside(09:00, 18:00)",
    "scope": "web.*"
  }

  필요: 자연어 입력 → 구조화된 정책 JSON → 사용자 확인 → 적용
  기존 P 섹션의 NL 정책을 확장하되, "일상 언어 → 시스템 정책" 변환 정확도 보장 메커니즘
```
...
### AC-2-1. [CRITICAL] 설계와 구현 사이의 갭

가장 큰 문제: 10,600줄의 설계서가 있지만 코드가 한 줄도 없다.

```
  보완 방향:
  ──────────────────────────────────────────────────────────────
  1. Phase 0의 구체적 파일 목록과 코드 스켈레톤을 설계서에 포함
  2. packages/core/types/index.ts에 핵심 타입을 먼저 코드로 구현
  3. 설계서의 TypeScript 인터페이스를 실제 컴파일 가능한 코드로 추출
  4. First Vertical Slice (hello.txt 시나리오)를 실제 구현하는 것이
     설계의 빈틈을 가장 빠르게 찾아준다.
```

### AC-2-2. [HIGH] 에이전트 간 통신 프로토콜 미정의

9개 에이전트가 있지만 에이전트 간 메시지 포맷/프로토콜이 명확하지 않다.

```typescript
// 필요한 에이전트 간 표준 메시지 포맷
...
```
  현재 상태:
  - Event Bus가 언급되지만 메시지 스키마 없음
  - 에이전트 간 어떤 데이터를 주고받는지 인터페이스만 부분적

  보완: AgentMessage 표준 스키마 정의, 에이전트 간 핸드셰이크 프로토콜,
        에러 전파 규칙, 타임아웃 처리 방식 명시
```

### AC-2-3. [HIGH] 오류 경계 및 우아한 성능 저하

에러 복구 플레이북(K-5)이 있지만, 시스템 전체가 부분 장애 시 어떻게 동작하는지 불명확하다.

```
  장애 시나리오                 필요한 대응 전략
  ──────────────────────────────────────────────────────────────
  Policy Engine 장애             모든 것을 DENY? 마지막 캐시 사용?
  State Machine DB 손상          복구 전략?
  에이전트 1개 지속 실패         해당 에이전트 우회 경로?
  UI 죽음 + Executor 실행 중     Emergency Stop 도달 방법?
  Claude API 전체 장애           로컬 폴백? 큐잉?
```

### AC-2-4. [HIGH] 프롬프트 엔지니어링 전략 부재

각 에이전트가 Claude API를 호출하지만, 실제 시스템 프롬프트/few-shot에 대한 설계가 없다.

```
  필요 항목                     설명
  ──────────────────────────────────────────────────────────────
  에이전트별 시스템 프롬프트      각 에이전트의 역할/제약/출력 형식 정의
  구조화된 출력 강제             JSON mode, tool use 패턴
  Prompt 버전 관리              정책처럼 버전업 관리
  프롬프트 인젝션 방어 구조      user content 분리, untrusted 태깅
  Few-shot 예시 라이브러리       에이전트별 좋은/나쁜 예시 세트
```

### AC-2-5. [MEDIUM] 보안 테스트 전략 부재

Threat Model(6개 공격 벡터)이 있지만, 이를 어떻게 테스트하는지 정의되지 않았다.

```
  필요 항목                     설명
  ──────────────────────────────────────────────────────────────
  Penetration Test 시나리오      각 위협 벡터별 공격 시뮬레이션
  Red Team 체크리스트            프롬프트 인젝션, 토큰 위조, 경로 탈출 등
  Fuzzing 전략                  Action API에 비정상 입력 테스트
  보안 CI 파이프라인             매 커밋마다 보안 규칙 자동 검증
```

### AC-2-6. [MEDIUM] 상태 머신 복잡도 관리

현재 상태 머신이 매우 복잡하다 (15+ 상태, 다수의 전이).

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  병렬 상태(parallel states)    Gate 대기 + Speculative Execution 동시 처리
  중첩 상태(nested states)      EXECUTING 내부의 sub-states 설계
  시각적 검증 도구              XState Inspector 활용 방안
  상태 폭발(state explosion)    방지 전략 (계층적 상태 분리)
```

### AC-2-7. [MEDIUM] 데이터 마이그레이션 전략

SQLite + JSON으로 데이터를 저장하지만 스키마 진화에 대한 전략이 없다.

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  스키마 버전 업그레이드          마이그레이션 스크립트 관리
  하위 호환성                   Pattern Cache, Workspace Profile 등 포맷 변경 대응
  백업/복원 CLI                 사용자 데이터 안전 관리
```

### AC-2-8. [MEDIUM] 동시성 설계 부재

Multi-Run 실행에서 "Conflict Detection"을 언급하지만 구체적이지 않다.

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  같은 파일 동시 쓰기            Lock 전략 (파일 레벨? Row 레벨?)
  SQLite WAL 모드               동시 쓰기 제한 (1 writer) 대응
  Executor 동시 앱 조작          UI 자동화 충돌 해결
```

### AC-2-9. [LOW] 국제화(i18n) 전략

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  Gate/에러 메시지 다국어        메시지 키 기반 번역 시스템
  음성 인식 다국어              한국어 ↔ 영어 전환
  정책 템플릿 다국어 버전        산업별 템플릿의 로컬라이즈
```

### AC-2-10. [LOW] 버전 관리 및 업데이트 전략

JARVIS OS 자체의 업데이트 메커니즘이 없다.

```
  항목                          설명
  ──────────────────────────────────────────────────────────────
  자동 vs 수동 업데이트          사용자 선택 가능
  업데이트 시 데이터 보존        기존 정책/패턴/메모리 유지
  롤백 가능한 업데이트           이전 버전으로 복원
```
...
현재 workflow.md가 10,600줄 단일 파일이므로 관리가 어렵다.

```
  docs/
  ├── architecture/
  │   ├── overview.md           # 아키텍처 개요
  │   ├── agents.md             # 9개 에이전트 정의
  │   ├── state-machine.md      # 상태 머신 설계
  │   └── action-api.md         # Action API 스펙
  ├── security/
  │   ├── threat-model.md       # 위협 모델
  │   ├── policy-engine.md      # 정책 엔진
  │   └── credential-vault.md   # 자격증명 관리
  ├── ui/
  │   ├── layout.md             # UI 레이아웃
  │   └── components.md         # 컴포넌트 목록
  ├── operations/
  │   ├── reporting.md          # 보고서 시스템
  │   └── compliance.md         # 규정 준수
  └── roadmap/
      ├── phases.md             # 구현 로드맵
      └── mvp-scenarios.md      # MVP 검증 시나리오
```
...
```
  우선순위    항목                              이유
  ──────────────────────────────────────────────────────────────
  즉시       First Vertical Slice 구현 시작     코드 없는 설계는 검증 불가
  P0         에이전트 간 통신 프로토콜 정의      구현의 핵심 기반
  P0         Prompt Engineering 전략            AI 에이전트 품질의 핵심
  P1         Error Boundary 설계               안정성의 핵심
  P1         보안 테스트 시나리오               Threat Model이 테스트 없이는 의미 없음
  P2         Plugin SDK                        확장성 확보
  P2         오프라인 폴백 전략                  실사용 환경 대비
  P3         접근성, 국제화                     사용자 기반 확대
```

---

# AD. OPENCLAW 스타일 실전 시나리오 3종

> 기존 J-4의 MVP 검증 시나리오가 "hello.txt 만들기" 같은 로컬 작업에 집중되어 있다.
> 아래 3종 시나리오는 **외부 서비스 통합(Slack, Notion, Telegram, API)**을 포함하는
> 현실적인 워크플로우이며, JARVIS OS의 정책 엔진, Gate 시스템, Capability Token,
> 에이전트 파이프라인이 실전에서 어떻게 작동하는지를 보여준다.

---

## AD-A. Slack 스레드 → 회의록 → Notion 게시 (L1 승인)

### 개요

```
  트리거:     사용자 텍스트 ("어제 #dev-standup 스레드 회의록 만들어줘")
  파이프라인:  Slack thread 읽기 → 요점/액션아이템 추출
              → Notion "회의록 템플릿" 페이지 생성 → Slack에 링크 공유
  Gate:       L1 (사용자가 초안 확인 후 게시 승인)
  Risk:       18 (LOW)
  예상 시간:   ~15초
  예상 비용:   ~$0.01
```
...
```
  사용자: "어제 #dev-standup 스레드 회의록 만들어줘"
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  1. Intent & Spec Agent (Haiku)                              │
  │                                                              │
  │  입력 분석: "Slack 스레드 읽기 + 요약 + Notion 게시"          │
  │                                                              │
  │  SPEC 산출:                                                  │
  │    · Source: Slack channel=#dev-standup, thread=latest        │
  │    · Output: Notion "회의록 템플릿" 페이지                    │
  │    · 추출 항목: 요점, 액션아이템(담당자+기한), 결정사항        │
  │                                                              │
  │  Acceptance Criteria:                                        │
  │    · 요점 3~7개 추출                                          │
  │    · 액션아이템에 담당자 태깅 포함                             │
  ...
  └──────────────────────────────────────────────────────────────┘
          ...
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  2. Policy & Risk Agent (Haiku)                              │
  │                                                              │
  │  Risk Score: 18 (LOW)                                        │
  │  근거: 읽기 전용(Slack) + 문서 생성(Notion) = 비파괴적        │
  │                                                              │
  │  Decision: ALLOW (Semi-Auto 모드에서 자동 실행 가능)           │
  │                                                              │
  │  Required Capabilities:                                      │
  │    cap: net:slack.api.read   scope: channel=#dev-standup     │
  │         TTL: 300s            max_uses: 1                     │
  │    cap: net:notion.api.write scope: database=회의록DB         │
  │         TTL: 600s            max_uses: 1                     │
  │    cap: net:slack.api.write  scope: channel=#dev-standup     │
  │         TTL: 60s             max_uses: 1 (링크 공유 전용)    │
  │                                                              │
  │  Constraints:                                                │
  │    · Slack DM 채널 접근 차단                                  │
  │    · Notion 회의록DB 외 접근 차단                              │
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  3. Planner Agent (Sonnet)                                   │
  │                                                              │
  │  PLAN.json:                                                  │
  │    Step 1: Slack API → conversations.replies() 호출           │
  │    Step 2: 스레드 메시지 파싱 (참여자, 시간, 내용 추출)       │
  │    Step 3: AI 요약 → 요점 + 액션아이템 + 결정사항 추출        │
  │    Step 4: Notion 회의록 템플릿에 페이지 생성                  │
  │    Step 5: Slack에 Notion 링크 공유                           │
  │                                                              │
  │  Budget: input ~3,000 tokens, output ~1,500 tokens            │
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  4. Codegen Agent (Opus)                                     │
  │                                                              │
  │  회의록 마크다운 생성:                                         │
  │                                                              │
  │  # 📋 Daily Standup 회의록                                    │
  │  **일시**: 2026-03-01 10:00                                   │
  │  **참여자**: Daniel, Sarah, Mike                              │
  │                                                              │
  │  ## 요점                                                     │
  │  1. 인증 모듈 리팩터링 완료 (Daniel)                          │
  │  2. API v2 엔드포인트 테스트 진행 중 (Sarah)                  │
  │  3. CI 파이프라인 최적화 검토 필요 (Mike)                     │
  │                                                              │
  │  ## 액션 아이템                                               │
  │  - [ ] API v2 통합 테스트 완료 → Sarah (기한: 03/03)         │
  │  - [ ] CI 벤치마크 결과 공유 → Mike (기한: 03/04)            │
  │                                                              │
  │  ## 결정사항                                                  │
  │  - 인증 모듈은 다음 스프린트에서 배포                          │
  └──────────────────────────────────────────────────────────────┘
  ...
  │  6. Executor Agent                                           │
  │                                                              │
  │  Action 1: Notion API → pages.create()                       │
  │    · 회의록 페이지 생성 → page_id 획득                        │
  │                                                              │
  │  Action 2: Slack API → chat.postMessage()                    │
  │    · "#dev-standup에 회의록이 생성되었습니다: [Notion 링크]"   │
  │                                                              │
  │  Evidence:                                                   │
  │    · 생성된 Notion 페이지 URL                                 │
  │    · Slack 메시지 타임스탬프                                   │
...
```
  자격증명              저장소              AI 접근 방식
  ──────────────────────────────────────────────────────────────
  Slack Bot Token       Credential Vault    세션 토큰으로 발급 (원문 비접근)
  Notion Integration    Credential Vault    세션 토큰으로 발급 (원문 비접근)

  로그 저장 금지: 토큰 원문, API 키, 쿠키
  로그 허용: credential_id, scope, 호출 시각
```
...
```json
{
  "run_id": "run_20260302_meetingnotes_001",
  "steps": [
    { "agent": "intent_spec", "action": "spec_draft", "duration_ms": 1200 },
    { "agent": "policy_risk", "action": "risk_eval", "risk_score": 18, "decision": "ALLOW" },
    { "agent": "planner", "action": "plan_create", "steps": 5 },
    { "agent": "codegen", "action": "markdown_gen", "output_tokens": 450 },
    { "agent": "gate", "type": "L1", "action": "user_approve", "wait_ms": 3200 },
    { "agent": "executor", "action": "notion_create", "page_id": "xxx" },
    { "agent": "executor", "action": "slack_post", "channel": "#dev-standup" }
  ],
  "total_tokens": 4200,
  "total_cost_usd": 0.01,
...
## AD-B. 텔레그램으로 "주간보고 만들어줘" (L1 승인)

### 개요

```
  트리거:     텔레그램 봇 커맨드 ("/weekly-report")
  파이프라인:  Telegram 명령 수신 → 지난 7일 Slack/Notion/GitHub/외부API 데이터 수집
              → 주간보고 초안 생성 → 사용자 확인 → Notion 게시
  Gate:       L1 (사용자가 초안 리뷰 후 게시 승인)
  Risk:       35 (MEDIUM) — 다중 API 접근 + 데이터 집계
  예상 시간:   ~45초
  예상 비용:   ~$0.04
```
...
```
  ...
  │  1. Intent & Spec Agent (Haiku)                              │
  │                                                              │
  │  입력 분석: 텔레그램 봇 커맨드 → 주간보고 생성 요청            │
  │                                                              │
  │  SPEC 산출:                                                  │
  │    · Period: 최근 7일 (2026-02-24 ~ 2026-03-02 자동 계산)    │
  │    · Sources:                                                │
  │      - Slack: 주요 채널 활동 요약                              │
  │      - Notion: 최근 변경 문서 목록                             │
  │      - GitHub: PR/커밋/이슈 통계                               │
  │      - External API: KPI 지표 (DAU, 에러율 등)                │
  │    · Output: Notion "주간보고" 페이지                          │
  │    · Format: 요약 / 주요성과 / 이슈 / 다음주계획 / 수치지표   │
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  ...
  │  근거: 다중 API 접근(4개) + 7일치 데이터 집계 = 범위가 넓음   │
  │                                                              │
  │  ⚠ MEDIUM이므로 반드시 사용자 승인 필요                       │
  │                                                              │
  │  Required Capabilities:                                      │
  │    cap: net:slack.api.read    scope: public-channels          │
  │         TTL: 600s             max_uses: 1                    │
  │    cap: net:notion.api        scope: read+write, 주간보고DB   │
  │         TTL: 900s             max_uses: 2 (조회+생성)        │
  │    cap: net:github.api.read   scope: org/repos               │
  │         TTL: 600s             max_uses: 1                    │
  │    cap: net:external.api.read scope: whitelist endpoints      │
  │         TTL: 300s             max_uses: 1                    │
  │                                                              │
  │  Constraints:                                                │
  │    · Slack DM/개인 채널 접근 차단                              │
  │    · Notion 급여/인사 관련 DB 접근 차단                        │
  │    · GitHub private repo 접근은 명시적 scope 내에서만          │
  │    · 외부 API 호출은 사전 등록된 엔드포인트만                   │
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  ...
  │  ┌──────────────────────────────────────────────┐            │
  │  │  Task A: Slack API                            │            │
  │  │  → 7일간 주요 채널(#general, #dev, #ops) 요약  │            │
  │  │                                              │            │
  │  │  Task B: Notion API                           │            │
  │  │  → 최근 7일 변경된 문서 목록                    │            │
  │  │                                              │            │
  │  │  Task C: GitHub API                           │            │
  │  │  → PR 머지 수, 커밋 수, 오픈 이슈 통계          │            │
  │  │                                              │            │
  │  │  Task D: External API                         │            │
  │  │  → KPI 지표 (DAU, 에러율, 배포 빈도)           │            │
  │  └──────────────────────────────────────────────┘            │
  │                                                              │
  │  Phase 2: 보고서 생성 (순차)                                  │
  │    Step 1: 데이터 통합 + 중복 제거 + 요약 압축                │
  │    Step 2: AI 요약 → 주요성과 / 이슈 / 계획 추출              │
  │    Step 3: 주간보고 마크다운 생성                              │
  │                                                              │
  │  Phase 3: 게시 (Gate 이후)                                    │
  │    Step 1: Notion 페이지 생성                                 │
  │    Step 2: 텔레그램에 완료 알림 + Notion 링크 전송             │
  │                                                              │
  │  Budget: input ~8,000 tokens, output ~3,000 tokens            │
  │  토큰 절감: 수집 데이터를 요약 후 Opus에 전달 (점진적 컨텍스트)│
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  ...
  │                                                              │
  │  ┌────────────────────────────────────────────────┐          │
  │  │  📊 주간보고 (2026.02.24 ~ 03.02)               │          │
  │  │                                                │          │
  │  │  ■ 이번 주 요약                                 │          │
  │  │    - 인증 모듈 v2 리팩터링 완료 및 스테이징 배포  │          │
  │  │    - API 응답 시간 p95 개선 (1.2s → 0.4s)       │          │
  │  │    - 신규 온보딩 워크플로우 설계 착수             │          │
  │  │                                                │          │
  │  │  ■ 주요 성과                                    │          │
  │  │    - GitHub: PR 12건 머지, 이슈 8건 해결         │          │
  │  │    - 배포: staging 3회, production 1회           │          │
  │  │    - Notion: 문서 15건 갱신                      │          │
  │  │                                                │          │
  │  │  ■ 이슈 & 블로커                                │          │
  │  │    - API 응답 지연 간헐 발생 (P2 추적 중)        │          │
  │  │    - CI 빌드 시간 증가 (5분 → 8분)              │          │
  │  │                                                │          │
  │  │  ■ 다음 주 계획                                 │          │
  │  │    - 인증 v2 프로덕션 배포 (03/05 예정)          │          │
  │  │    - CI 파이프라인 최적화                        │          │
  │  │    - 온보딩 워크플로우 MVP 완성                   │          │
  │  │                                                │          │
  │  │  ■ 수치 지표                                    │          │
  ...
  │  │  📊 주간보고 초안이 준비되었습니다.              │          │
  │  │                                                │          │
  │  │  요약: 성과 5건, 이슈 2건, 계획 3건              │          │
  │  │  데이터 소스: Slack + Notion + GitHub + API      │          │
  │  │  게시 대상: Notion "주간보고" DB                  │          │
  │  │  Risk: MEDIUM (35)                             │          │
  │  │                                                │          │
  │  │  [전문 보기]                                    │          │
  │  │  [승인] [수정 요청] [거부]                       │          │
  ...
  │                                                              │
  │  수정 요청 시:                                                │
  │    사용자: "이슈 섹션에 DB 마이그레이션 지연 건 추가해줘"       │
  │    → Codegen이 수정 반영 → 재승인 요청                         │
  └──────────────────────────────────────────────────────────────┘
          │ 승인
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  ...
  │                                                              │
  │  Action 1: Notion API → pages.create()                       │
  │    · 주간보고 페이지 생성 → page_id 획득                      │
  │                                                              │
  │  Action 2: Telegram Bot API → sendMessage()                  │
  │    · "주간보고가 게시되었습니다: [Notion 링크]"                 │
  │                                                              │
  │  Evidence:                                                   │
  │    · Notion 페이지 URL                                       │
  │    · 수집 데이터 소스 목록 (4개 API)                           │
...
```
  고려사항                      대응
  ──────────────────────────────────────────────────────────────
  데이터 소스 4개 병렬 접근      Planner가 Phase 1을 병렬 Task로 설계
  개인정보 필터링                Policy가 DM/인사DB 접근 자동 차단
  대용량 데이터                  7일 윈도우 + 채널 필터로 범위 제한
  토큰 예산                     수집 데이터를 요약 후 Opus에 전달 (점진적 컨텍스트)
  텔레그램 UX                   초안을 인라인 메시지로 보여주고 버튼으로 승인/수정
  반복 실행 최적화               Pattern Cache에 "주간보고" 패턴 저장 → 2회차부터 자동 제안
```

### 권한 토큰 상세

```
  Token                    Scope                        TTL    Max Uses
  ──────────────────────────────────────────────────────────────────────
  net:slack.api.read       public-channels, 7일 범위     600s   1
  net:notion.api.read      주간보고DB + 최근문서 목록     900s   1
  net:notion.api.write     주간보고DB (페이지 생성)       900s   1
  net:github.api.read      org/repos (통계 조회)         600s   1
  net:external.api.read    whitelist endpoints           300s   1
  net:telegram.bot.write   sendMessage (알림)            60s    1
```

---

## AD-C. 외부 API 상태 점검 + 문서화 (L1 + L2)

### 개요

```
  트리거:     스케줄 (매 시간) 또는 수동 명령
  파이프라인:  API.CALL로 장애/지표 조회 → 요약 + 원인 추정(근거 포함)
              → Runbook 업데이트(Notion) → Slack 알림
  Gate:       L1 = 문서화 + 알림 (LOW risk, 자동/승인)
              L2 = "자동 복구/재시작" 같은 파괴적 행동일 때만 (HIGH risk)
  Risk:       25 (L1) ~ 78 (L2)
  예상 시간:   ~20초 (L1), ~3분 (L2 포함)
  예상 비용:   ~$0.02 (L1), ~$0.06 (L2 포함)
```

### 에이전트 파이프라인 상세

```
  Trigger: 스케줄 또는 "API 상태 점검해줘"
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  1. Data Collection (Executor → 규칙 기반)                    │
  │                                                              │
  │  API.CALL: 외부 서비스 health endpoint 호출                    │
  │                                                              │
  │  수집 데이터:                                                  │
  │    · HTTP status codes (각 엔드포인트)                         │
  │    · Response time: p50=120ms, p95=3200ms, p99=5100ms        │
  │    · Error rate: 2.4% (최근 1시간)                            │
  │    · 인프라 메트릭: CPU 72%, Memory 85%, Disk I/O 정상        │
  │    · Active DB connections: 48/50 (96% 사용률)                │
  │    · 최근 배포 이력: 30분 전 deploy-prod 기록                  │
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  ...
  │  │                                                │          │
  │  │  증상:                                          │          │
  │  │  - API 응답 시간 p95 = 3.2s (정상 기준: <500ms) │          │
  │  │  - Error rate 2.4% (정상 기준: <0.5%)          │          │
  │  │  - DB connection pool 거의 고갈                  │          │
  │  │                                                │          │
  │  │  원인 추정 (근거 포함):                          │          │
  │  │  1. DB connection pool 고갈 (확률 85%)          │          │
  │  │     근거: active_connections=48/50              │          │
  │  │           느린 쿼리 3건 감지 (>2s)              │          │
  │  │                                                │          │
  │  │  2. 최근 배포 영향 가능 (확률 40%)               │          │
  │  │     근거: 30분 전 deploy-prod 기록               │          │
  │  │           배포 전후 p95 비교: 400ms → 3200ms    │          │
  │  │                                                │          │
  │  │  권장 조치:                                      │          │
  │  │  - 즉시: connection pool flush + size 증설       │          │
  │  │  - 단기: 느린 쿼리 3건 최적화                    │          │
  │  │  - 장기: read replica 도입 검토                  │          │
  │  └────────────────────────────────────────────────┘          │
  └──────────────────────────────────────────────────────────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  3. Policy & Risk Agent (Haiku)                              │
  │                                                              │
  │  ★ 핵심: 여기서 L1/L2 경로가 분기된다.                         │
  │                                                              │
  │  ┌────────────────────────────────────────────────┐          │
  │  │                                                │          │
  │  │  경로 A) 문서화 + 알림                          │          │
  │  │    Risk Score: 25 (LOW)                        │          │
  │  │    → L1 승인 또는 Semi-Auto 자동 실행           │          │
  │  │    비파괴적: Notion 페이지 업데이트 + Slack 알림  │          │
  │  │                                                │          │
  │  │  경로 B) 자동 복구 실행                         │          │
  │  │    Risk Score: 78 (HIGH)                       │          │
  │  │    → L2 반드시 사용자 승인 필수                  │          │
  ...
  │  └────────────────────────────────────────────────┘          │
  │                                                              │
  │  Required Capabilities (경로 A):                             │
  │    cap: net:notion.api.write  scope: RunbookDB    TTL: 600s  │
  │    cap: net:slack.api.write   scope: #ops-alerts  TTL: 60s   │
  │                                                              │
  │  Required Capabilities (경로 B 추가):                        │
  │    cap: infra:service.restart scope: api-server   TTL: 120s  │
  │    cap: infra:db.flush        scope: conn-pool    TTL: 60s   │
  │    cap: infra:deploy.rollback scope: api-server   TTL: 120s  │
  └──────────────────────────────────────────────────────────────┘
          │
          ├──── 경로 A ────────────────────────────────────┐
          │                                                │
          ▼                                                │
  ┌──────────────────────────────────────────────────────┐ │
  │  4a. Executor (L1 경로 — 문서화 + 알림)               │ │
  │                                                      │ │
  │  Action 1: Notion API → Runbook 페이지 업데이트       │ │
  │    · 장애 타임라인 추가                                │ │
  │    · 원인 분석 기록 (근거 포함)                        │ │
  │    · 조치 이력 업데이트                                │ │
  │    · 관련 메트릭 스냅샷 첨부                           │ │
  │                                                      │ │
  │  Action 2: Slack API → #ops-alerts 채널에 알림        │ │
  │    ┌──────────────────────────────────────┐          │ │
  │    │  ⚠ API 상태: DEGRADED                │          │ │
  │    │                                      │          │ │
  │    │  p95 응답: 3.2s (정상 <500ms)        │          │ │
  │    │  에러율: 2.4% (정상 <0.5%)           │          │ │
  │    │  추정 원인: DB connection pool 고갈   │          │ │
  │    │                                      │          │ │
  │    │  Runbook: [Notion 링크]              │          │ │
  │    │  권장 조치: connection pool flush     │          │ │
  │    │                                      │          │ │
  │    │  [자동 복구 요청] [수동 처리]         │          │ │
  │    └──────────────────────────────────────┘          │ │
  │                                                      │ │
  ...
  └──────────────────────────────────────────────────────┘ │
                                                           │
          ├──── 경로 B (사용자가 자동 복구 요청 시) ──────────┘
          │
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  4b. Recovery 필요 여부 판단                                   │
  │                                                              │
  │                    ┌─────────────┐                           │
  │                    │ 복구 필요?   │                           │
  │                    └──────┬──────┘                           │
  ...
  │  │  ★ GATE L2: 자동 복구 승인 요청                  │         │
  │  │                                                │         │
  │  │  🚨 이 작업은 프로덕션 서비스에 영향을 줍니다.   │         │
  │  │                                                │         │
  │  │  대상: api-server-prod (3 instances)            │         │
  │  │  작업: Connection pool flush + 서비스 재시작     │         │
  │  │  영향: 약 10초간 서비스 중단 (rolling restart)   │         │
  │  │  Risk: 78 (HIGH)                               │         │
  │  │                                                │         │
  │  │  ⚠ Auto 모드에서도 이 Gate는 필수입니다.        │         │
  │  │                                                │         │
  │  │  복구 계획:                                     │         │
  │  │  1. 체크포인트 저장 (현재 상태 스냅샷)           │         │
  │  │  2. Connection pool flush                      │         │
  │  │  3. 서비스 graceful restart (rolling)           │         │
  │  │  4. Health check 재확인 (30초 대기)             │         │
  │  │  5. 실패 시 즉시 롤백                           │         │
  │  │                                                │         │
  │  │  [승인: 복구 실행] [거부: 수동 처리] [연기]      │         │
  │  └────────────────────────────────────────────────┘         │
  └──────────────────────────────────────────────────────────────┘
          │ 승인
          ▼
  ┌──────────────────────────────────────────────────────────────┐
  │  5b. Executor + Recovery Agent (L2 경로 — 복구 실행)          │
  │                                                              │
  │  Pre-execution:                                              │
  │    · Recovery Agent 활성화                                    │
  │    · 체크포인트 저장 (현재 시스템 상태 스냅샷)                 │
  │    · Rollback 계획 준비                                       │
  │                                                              │
  │  Execution:                                                  │
  │    Step 1: Connection pool flush                             │
  │      → 기존 연결 해제, 새 pool 초기화                          │
  │    Step 2: Service graceful restart (rolling)                │
  │      → instance-1 restart → health ok → instance-2 → ...    │
  │    Step 3: Health check 재확인                                │
  │      → p95 응답 시간 확인, error rate 확인                    │
  │                                                              │
  │  Post-execution:                                             │
  │    · 성공 시:                                                 │
  │      - Runbook에 복구 이력 추가                               │
  │      - Slack에 "복구 완료: p95 3.2s → 380ms" 알림             │
  │      - 체크포인트 해제                                        │
  │    · 실패 시:                                                 │
  │      - 즉시 롤백 (체크포인트에서 복원)                         │
  │      - Slack에 "자동 복구 실패, 수동 개입 필요" 긴급 알림     │
  │      - CRITICAL_ERROR 상태 전이                               │
  │      - Runbook에 실패 이력 기록                               │
  │                                                              │
  │  Evidence:                                                   │
  │    · 복구 전/후 메트릭 비교 스냅샷                             │
...
```
  ...
  ├────────────────────────────────────────────────────────────────┤
  │                                                                │
  │  행동 유형               Risk    Level    Gate                  │
  │  ──────────────────────────────────────────────────────────    │
  │  Runbook 문서 업데이트    15      LOW      L1 (Semi-Auto 자동)  │
  │  Slack 알림 전송          10      LOW      L1 (Semi-Auto 자동)  │
  │  로그 레벨 변경           20      LOW      L1 (자동 가능)       │
  │  캐시 무효화 (flush)      55      MED      L1 (Gate 권장)       │
  │  설정값 변경              45      MED      L1 (Gate 권장)       │
  │  ──────────────────────────────────────── L2 라인 ──────────   │
  │  Connection pool flush    65      HIGH     L2 (필수 승인)       │
  │  인스턴스 스케일 조정      70      HIGH     L2 (필수 승인)       │
  │  서비스 재시작             78      HIGH     L2 (필수 승인)       │
  │  배포 롤백                82      HIGH     L2 (필수 승인)       │
  │  DNS 변경                 90      HIGH     L2 (필수 승인)       │
  │                                                                │
  │  규칙:                                                         │
  │  - L2는 Auto 모드에서도 Gate 필수 (예외 없음)                   │
  │  - L2 연속 3회 이상 시: 자동 빈도 제한 (1분 쿨다운)             │
  │  - L2 실패 시: 즉시 롤백 + CRITICAL_ERROR                      │
  └────────────────────────────────────────────────────────────────┘
```
...
```
  경로 A (문서화 + 알림):
  ──────────────────────────────────────────────────────────────────
  Token                      Scope                   TTL    Uses
  net:external.api.read      health endpoints        300s   1
  net:notion.api.write       RunbookDB               600s   1
  net:slack.api.write        #ops-alerts             60s    1

  경로 B (복구 실행 — 추가):
  ──────────────────────────────────────────────────────────────────
  Token                      Scope                   TTL    Uses
  infra:service.restart      api-server-prod         120s   3 (rolling)
  infra:db.flush             connection-pool         60s    1
  infra:deploy.rollback      api-server-prod         120s   1
```
...
```
  항목              A: Slack 회의록    B: 주간보고        C: API 점검
  ──────────────────────────────────────────────────────────────────
  트리거            사용자 텍스트      텔레그램 커맨드     스케줄/수동
  데이터 소스       Slack 1개          4개 (병렬 수집)    외부 API + 인프라
  Risk Score        18 (LOW)          35 (MEDIUM)        25~78 (분기)
  Gate Level        L1                L1                 L1 + L2 (조건부)
  주요 에이전트     Spec→Codegen      Spec→Planner→      Spec→Analysis→
                                     Codegen            Recovery
  Capability 수     3개               6개                3~6개 (분기)
  파괴적 행동       없음              없음               L2에서 서비스 재시작
  출력              Notion + Slack    Notion + Telegram  Notion + Slack (+복구)
  예상 시간         ~15초             ~45초              ~20s (L1), ~3min (L2)
  예상 비용         ~$0.01            ~$0.04             ~$0.02 (L1) ~$0.06 (L2)
  패턴 캐시 활용    높음 (반복 가능)   높음 (매주 반복)   중간 (장애 시에만)
```

### 시나리오가 검증하는 JARVIS OS 기능

```
  JARVIS OS 기능                       A    B    C
  ──────────────────────────────────────────────────
  외부 API Capability Token             ✅   ✅   ✅
  Credential Vault 경유                ✅   ✅   ✅
  Policy Risk Score 분기               ○    ✅   ✅
  L1 Gate (승인 후 게시)               ✅   ✅   ✅
  L2 Gate (파괴적 행동 승인)           ○    ○    ✅
  병렬 데이터 수집                      ○    ✅   ○
  Recovery Agent 활성화                 ○    ○    ✅
...
```
  기존 J-4 MVP 시나리오 (hello.txt 등)와의 차이:
  ──────────────────────────────────────────────────────────────

  1. 외부 서비스 통합
     기존: 로컬 파일 시스템 조작 위주
     신규: Slack, Notion, Telegram, GitHub, 외부 API 실제 연동

  2. 복합 데이터 흐름
     기존: 단일 소스 → 단일 출력
     신규: 다중 소스 병렬 수집 → 집계 → 다중 출력

  3. 조건부 Gate 분기 (L1/L2)
     기존: 단순 승인/거부
     신규: 같은 워크플로우 안에서 위험도에 따라 Gate 레벨 동적 변경

  4. 비가역 외부 부작용
     기존: 로컬 파일 변경 (되돌리기 가능)
     신규: Notion 게시, Slack 메시지, 서비스 재시작 (되돌리기 불가)

  5. 실제 운영 시나리오
     기존: 개발자 도구 자동화
     신규: DevOps/팀 협업/장애 대응 워크플로우
```
